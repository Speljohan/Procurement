...
namespace POEApi.Model
{
    public static class  Settings
    {
        ...
        public static Dictionary<int, ShopEntryInfo> Buyouts { get; private set; }
        ...
        static Settings()
        {
            ...
            try
            {
                Buyouts = new Dictionary<int, ShopEntryInfo>();
                if (originalDoc.Element("Buyouts") != null)

                    Buyouts = originalDoc.Element("Buyouts").Elements("Item").ToDictionary(list => (int)list.Attribute("id"), list => new ShopEntryInfo((int)list.Attribute("id"), list.Attribute("buyoutValue").Value, list.Attribute("currentOffer").Value, (bool)list.Attribute("highlight"), (DateTime)list.Attribute("lastUpdated")));
            }
            catch (Exception ex)
            {
        ...
        public static void Save()
        {
            ...
            foreach (ShopEntryInfo info in Buyouts.Values)
            {
                XElement buyout = new XElement("Item", new XAttribute("id", info.Id), new XAttribute("buyoutValue", info.Buyout), new XAttribute("currentOffer", info.CurrentOffer), new XAttribute("highlight", info.Highlight), new XAttribute("lastUpdated", info.LastUpdated));
