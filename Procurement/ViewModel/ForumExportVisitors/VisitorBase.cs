...
namespace Procurement.ViewModel.ForumExportVisitors
{
    internal abstract class VisitorBase : IVisitor
    {
        ...
        protected string getLinkItem<T>(T item) where T : Item
        {
            ...
            {
                bool hasOffer = Settings.Buyouts[item.UniqueIDHash].CurrentOffer != string.Empty;
                if (hasOffer)
                {
                    return string.Format("\n[linkItem location=\"{0}\" league=\"{1}\" x=\"{2}\" y=\"{3}\"]\n~b/o {4}\n~c/o {5}\n", item.inventoryId, ApplicationState.CurrentLeague, item.X, item.Y, Settings.Buyouts[item.UniqueIDHash].Buyout, Settings.Buyouts[item.UniqueIDHash].CurrentOffer);
                }
                else
                {
                    return string.Format("\n[linkItem location=\"{0}\" league=\"{1}\" x=\"{2}\" y=\"{3}\"]\n~b/o {4}\n", item.inventoryId, ApplicationState.CurrentLeague, item.X, item.Y, Settings.Buyouts[item.UniqueIDHash].Buyout);
                }
            }
